---
hide:
  - toc
---

# 17. Mount the unplugged power strip

<p class="pf-step-count">Assembly · Step 17 of 19</p>

<p class="pf-profile-banner" data-guide-device="trimui-smart-pro" data-layout-id="chassis-core-v2"><strong>Compatible DUT:</strong> TrimUI Smart Pro <span>·</span> <code>chassis-core-v2</code></p>

Put the strip front-to-back inside the lower rail on the operator's right. It
must remain visibly unplugged throughout this mechanical step.

<div class="pf-step-layout" markdown="1">
<div class="pf-step-visual">
  <figure>
    <img src="../../../../../../assets/generated/test-node-chassis/legacy/detail-08-power-strip.png" alt="Unplugged power strip highlighted front-to-back inside the lower operator-right depth rail on two printed mounting blocks">
    <figcaption>The strip follows the right depth rail. Its switch remains reachable from the operator side and every mounting screw enters a printed block.</figcaption>
  </figure>
</div>
<div class="pf-step-copy" markdown="1">

## Get these parts

<ul class="pf-part-list">
  <li><span class="pf-part-tag pf-part-tag--printed">Printed</span><span><strong>2 × power-strip mounting blocks</strong><small>Core 04 frame hardware</small></span></li>
  <li><span class="pf-part-tag pf-part-tag--fastener">M3</span><span><strong>4 × shortest suitable M3 flat-head screws</strong><small>Use the four active channel bars loaded in <code>DEPTH-R-L</code> during Step 3</small></span></li>
  <li><span class="pf-part-tag pf-part-tag--supplied">Supplied</span><span><strong>4 × wall-mount screws from the strip</strong><small>They enter printed pilot holes only</small></span></li>
  <li><span class="pf-part-tag pf-part-tag--unpowered">Unplugged</span><span><strong>1 × accepted power strip</strong><small>Keep its mains plug disconnected and visible</small></span></li>
  <li><span class="pf-part-tag pf-part-tag--tool">Tools</span><span><strong>Matching driver, ruler, and optional pilot bit</strong><small>Drill printed ABS only if the supplied screws require it</small></span></li>
</ul>

## Do this

1. Put the disconnected mains plug where you can see it.
2. Stand at the operator side and identify the lower depth rail on your right,
   `DEPTH-R-L`.
3. Move the four active channel bars in its inside groove into two pairs. Keep
   both pairs clear of the gantry joint bar and parked blue-tagged spare.
4. Hold both printed blocks against that inboard rail face and loosely fasten
   each block to one pair of channel bars.
5. Measure the actual strip's wall-keyhole locations. Slide and rotate the
   blocks until all four printed pilots match those locations and the strip
   will run operator-to-device.
6. If the supplied wall screws need larger pilots, remove the blocks and drill
   **only the printed ABS** to the screws' root diameter.
7. Drive the four supplied screws into the printed blocks, leaving their heads
   at the strip's keyhole-retention gap.
8. Reinstall the blocks if removed, hang the unplugged strip on the four screw
   heads, then hand-tighten the M3 rail screws.
9. Confirm the strip enclosure does not touch the aluminum and its switch and
   mains plug remain accessible.

!!! danger "Never drill or pierce powered hardware"
    Drill only a removed printed block. Never drill the strip enclosure or an
    aluminum rail, and never work with the strip connected to mains.

</div>
</div>

<div class="pf-step-check" markdown="1">

**Before you continue:** the unplugged strip runs front-to-back inside
`DEPTH-R-L`, all four supplied screws enter printed ABS only, the enclosure is
undamaged and clear of aluminum, and the switch remains reachable.

</div>

<nav class="pf-step-nav" aria-label="Assembly step navigation">
  <a href="../16-add-placard/">← Node placard</a>
  <span>Step 17 of 19</span>
  <a href="../18-add-stacking-tabs/">Next: Stacking tabs →</a>
</nav>
