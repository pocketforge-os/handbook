...
<p class="pf-profile-banner" data-guide-device="trimui-smart-pro" data-layout-id="chassis-core-v2"><strong>Compatible DUT:</strong> TrimUI Smart Pro <span>·</span> <strong>Layout:</strong> <code>chassis-core-v2</code> <span>·</span> candidate</p>
...
This source revision is a candidate until those newly printed links receive
owner fit acceptance; the former <code>chassis-core-v1</code> record remains
the immutable qualified baseline.
...
| Qualification | `candidate` · print and owner fit acceptance pending |
...
cross-checks this guide against the published device-pack catalog. The v2 pack
is deliberately non-production: source validation does not extend the v1
physical qualification to newly printed links.

Next: [collect the candidate parts and tools](parts.md).
